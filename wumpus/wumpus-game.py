...
def create_map_file(file_name):
        image_list = ['pit_image','wumpus_image']
        map_file = open(file_name,"w")
        selected_cave = []
        gold_cave_number  = 100
        for i in range(1,5):
                random_image = image_list[randrange(0,2)]
        ...
        while True:
                random_cave = randrange(2,100)
                gold_cave_number = random_cave
                if (random_cave in selected_cave) == False:
                        break
        
        cave_str = str('gold_image'+','+str(gold_cave_number))
        map_file.write(cave_str)
        map_file.close()
...
def keep_map_alive_and_update():
        ...
        player = Player(all_map_percept_list,startPosition)
        agent = Agent(10,10)
        ...
        while True:
                ...
                agent.beforeMove(player)
                sets = agent.getAction(player)
                nextactions = sets[0]
                nextMove = sets[1]
                # print(actions)
                # move_agent(list_of_caves[randrange(100)][0],list_of_caves[randrange(100)][1])
                player.setAction(nextactions,nextMove)
                # newMove = actions[-1]
                str_next = str(nextMove[0])+str(nextMove[1])
                #str_next = str_next.rfill()
                index_next = int(str_next)
                #index_next = index_next

                move_agent(list_of_caves[index_next][0],list_of_caves[index_next][1])
                ...
                if(all_map_percept_list[nextMove[0]][nextMove[1]].glitter == True):
                        print("Game Over!!!")
